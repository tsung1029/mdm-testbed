...
! default unit numbers
      ...
      real :: init_time, loop_time
      real, dimension(:,:), pointer :: part, part2, parti, parti2
      real, dimension(:,:), pointer :: vpart
      real, dimension(:,:), pointer :: qe, qi
      real, dimension(:,:,:), pointer :: cu, amu, fxyze, bxyze, vcu, fpar
...
! text output file
      ...
      temp_mdm = system("mkdir MS/CURRENT");
      temp_mdm = system("mkdir MS/CURRENT/electron");
      temp_mdm = system("mkdir MS/CURRENT/electron/j1");
      temp_mdm = system("mkdir MS/CURRENT/electron/j2");
      temp_mdm = system("mkdir MS/CURRENT/electron/j3");
      temp_mdm = system("mkdir MS/CURRENT/electron/j1line");
      temp_mdm = system("mkdir MS/CURRENT/electron/j2line");
      temp_mdm = system("mkdir MS/CURRENT/electron/j3line");
      temp_mdm = system("mkdir MS/CURRENT/total");
      temp_mdm = system("mkdir MS/CURRENT/total/j1");
      temp_mdm = system("mkdir MS/CURRENT/total/j2");
      temp_mdm = system("mkdir MS/CURRENT/total/j3");
      temp_mdm = system("mkdir MS/CURRENT/total/j1line");
      temp_mdm = system("mkdir MS/CURRENT/total/j2line");
      temp_mdm = system("mkdir MS/CURRENT/total/j3line");
      iuot = get_funit(iuot)
...
!allocate vpml arrays and set up coefficients if needed
      if(vpml.ge.1) then
        call setup_vpml(vpmlx_e_up,vpmlx_e_low,vpmly_e_up, &
     & vpmly_e_low,vpmlx_b_up,vpmlx_b_low,vpmly_b_up,vpmly_b_low, &
     & coef_e_low,coef_e_up,coef_b_low,coef_b_up,vpml_exyz,vpml_bxyz, &
     & vpml,xtrue,ytrue,nx,ny,nxe,nye,dt,ci)
      endif
! ffc = form factor array for poisson solver
      allocate(ffc(nxeh,nyh))
! mixup, sct = arrays for fft
      allocate(mixup(nxhy),sct(nxyh))
! virtual particle current array
      if(parmax==1) then
        allocate(vpart(idimp+4,4*np)) !!!
        if(inorder==1) then
          allocate(vcu(3,nx+3,ny+3))
        else if (inorder==2) then
          allocate(vcu(3,nx+4,ny+4))
        else if (inorder==3) then
          allocate(vcu(3,nx+5,ny+5))
        endif
        print*,'using charge conserving current deposit'
      endif
      print *,'number of electron = ', np
      if (movion==1) print *,'number of ion = ', npi
!debug parallel force array
      allocate(fpar(2,nxe,nye))
!
...
!
  500 if (nloop <= ntime) go to 2000
      ...
      !part(5,1) = vosc*sin(dt*real(ntime))
!
...
! make a dipole oscillating in x (radiating) for debugging
      !call dipole_ant(part,parti,0.5236,2.0,dt,ntime,140,np,npi,nx,ny,idimp)
! make a single particle oscillating in z (radiating) for debugging
      !call partant(part,2,40,dt,ci,ntime,np,nx,ny,idimp)
! deposit electron current
      if(parmax==1) then
      vpart = 0.0
      cu = 0.0
      vcu = 0.0
      call odjpostg(part,vpart,cu,vcu,np,qme,dth,ci,tdjpost,nx,ny,ipbc,&
     &relativity,inorder,djopt,dt)
!      print*, '---'
! add guard cells from virtual current array.
! needs to be done before electron current diagnostic.
      call avcguard(cu,vcu,nx,ny,inorder)
      else
...
! Electron current diagnostic
     call ECurdiag(cu,ntime,ntej1,ntej2,ntej3,ntlej1,ntlej2,ntlej3, & 
    &nxe,nye,inorder)
...
! add ion current and deposit ion charge density
      if (movion==1) then
         ...
         vpart = 0.0
         vcu = 0.0
         call odjpostg(parti,vpart,cu,vcu,npi,qmi,dth,ci,tdjpost,nx,ny,ipbc,&
     & relativity,inorder,djopt,dt)
         call avcguard(cu,vcu,nx,ny,inorder)
         else
...
! save ion current
      ...
      if(parmax==1) then
! shift to match Viktor's current
        if(inorder==1) then
          cu = cu
        else if(inorder==2) then
          cu(1,2:nx+1,2:ny+1) = cshift(cshift(cu(1,2:nx+1,2:ny+1),shift = -1,dim = 2)&
     &,shift = -1,dim = 1)
          cu(2,2:nx+1,2:ny+1) = cshift(cshift(cu(2,2:nx+1,2:ny+1),shift = -1,dim = 2)&
     &,shift = -1,dim = 1)
          cu(3,2:nx+1,2:ny+1) = cshift(cshift(cu(3,2:nx+1,2:ny+1),shift = -1,dim = 2)&
     &,shift = -1,dim = 1)
      elseif(inorder==3) then
        cu(1,3:nx+2,3:ny+2) = cshift(cshift(cu(1,3:nx+2,3:ny+2),shift = 1,dim = 2)&
    &,shift = 1, dim = 1)
        cu(2,3:nx+2,3:ny+2) = cshift(cshift(cu(2,3:nx+2,3:ny+2),shift = 1,dim = 2)&
    &,shift = 1,dim = 1)       
        cu(3,3:nx+2,3:ny+2) = cshift(cshift(cu(3,3:nx+2,3:ny+2),shift = 1,dim = 2)&
    &,shift = 1,dim = 1)       
        endif
      else
! add guard cells for viktor's current
        call acguard(cu,nx,ny,inorder)
      endif
! current antenna for debugging
      !call cuant(cu,2,40,dt,ci,ntime,nxe,nye,inorder)
      !print*, cu(3,nx/2-4:nx/2+4,ny/2-4:ny/2+4)
      if(movion==1) then
        call TCurdiag(cu,ntime,nttj1,nttj2,nttj3,ntltj1,ntltj2,ntltj3, & 
     &nxe,nye,inorder)
      endif
! add electron and ion densities
      ...
      !print*, qe(nx/2-4:nx/2+4,ny/2-4:ny/2+4)
...
! transform current to fourier space
      ...
      if(parmax==1) then
        if(yee==0) then
! shift current so it's at the grid points (where E is)
          call cushift(cu,nx,ny,inorder)
        endif
      else
...
! calculate initial darwin magnetic field
         call ibpois(cu,bxyz,ffc,ci,wm,nx,ny,inorder,yee)
         wf = 0.
         ! save current soub
...
!        call iwfft2rn(amu,isign,mixup,sct,tfft,indx,indy,inorder)
         ...
         call iepois(bxyze,exyz,ffe,ci,wf,nx,ny,inorder,yee)

         !    SAVCUZ2(cu,cu0,nxvh,nyv)
         call isavcuz2(cu,cu0,inorder)
! initialize a bare bones plane wave for debugging
!         isign = -1
!         call initpw2(fxyze,bxyze,1,512,nx,ny,nxe,nye,1.0,ci,isign)
!         print*, fxyze(3,1,:)
!         print*, fxyze(3,nx,:)
!         print*, bxyze(2,1,:)
!         print*, bxyze(2,nx,:)
...
!         call ilsremfield2(fxyze,exyz,ffc,1,nx,ny,inorder)
!         call ilsremfield2(bxyze,bxyz,ffc,1,nx,ny,inorder)
         deallocate(amu,ffe)
         dth = .5*dt
        if(parmax==1) then
! initialize E parallel
!*****
!commented out 06.10.2015 for debugging PML
           !call pois3(qe,fxyze,ffc,we,tfield,nx,ny,inorder,yee)
!*****
! add fxyze to exyz
           isign = 0
...
! calculate electromagnetic fields
       ...
       if(yee==1) then
! shift the fields to their yee mesh positions
         isign = 1
!         call fldshift(fxyze,exyz,bxyz,isign,nx,ny)
       endif
       if(vpml.ge.1) then
         !update B a half time step
         if(yee==1) then
           call maxwelbyee(exyz,bxyz,cu,ffc,ci,dt,wf,wm,tfield,nx,ny,inorder)
         else
          call maxwelb(exyz,bxyz,ci,dt,wm,tfield,nx,ny,nxe,nye,nxeh,nyh, &
               inorder)
! shift the box EM fields to their Yee positions
           isign = 1
           call fldshift(fxyze,exyz,bxyz,isign,nx,ny)
         endif
         ! copy the box fields to a temporary array that we'll use to communicate
         ! the box fields to the VPML fields
         vpml_exyz = 0.0
         vpml_bxyz = 0.0
         isign = -1
         call emfield(vpml_bxyz,bxyz,ffc,isign,nx,ny,inorder)
         ! transform those fields to real space
         isign = 1
         call fft(vpml_bxyz,isign,mixup,sct,tfft,indx,indy,inorder) 
         !call pulse_fix(vpml_bxyz,lasstart,lasend,nxe,nye,ntime,inorder,ci,dt)
         ! update VPML fields
         call update_e_vpml(vpmlx_e_up,vpmlx_e_low,vpmly_e_up, &
     & vpmly_e_low,vpmlx_b_up,vpmlx_b_low,vpmly_b_up,vpmly_b_low, &
     & vpml_bxyz,coef_e_low,coef_e_up,xtrue,ytrue,dt,ci,&
     & inorder)
         ! transform the updated temp. box to Fourier space
         isign = -1
         call fft(vpml_bxyz,isign,mixup,sct,tfft,indx,indy,inorder)
         ! paste the temp. box the actual box field array
         bxyz = 0.0
         call ilsremfield2(vpml_bxyz,bxyz,ffc,1,nx,ny,inorder)
         ! update E a full time step
         if(yee==1) then
           call maxweleyee(exyz,bxyz,cu,ffc,ci,dt,wf,wm,tfield,nx,ny,inorder,parmax)
         else
! shift the box array back to the integer grid points
           isign = -1
           call fldshift(fxyze,exyz,bxyz,isign,nx,ny)
           call maxwele(exyz,bxyz,cu,ffc,ci,dt,wf,wm,tfield,nx,ny,inorder,parmax)
! shift the box EM fields to their Yee positions
           isign = 1
           call fldshift(fxyze,exyz,bxyz,isign,nx,ny)
         endif
         ...
         vpml_bxyz = 0.0
         isign = -1
         call emfield(vpml_exyz,exyz,ffc,isign,nx,ny,inorder)
         ! transform those fields to real space
         isign = 1
         call fft(vpml_exyz,isign,mixup,sct,tfft,indx,indy,inorder)
!         !call pulse_fix(vpml_exyz,lasstart,lasend,nxe,nye,ntime,inorder,ci,dt)
!         ! update VPML fields
         call update_b_vpml(vpmlx_b_up,vpmlx_b_low,vpmly_b_up, &
        & vpmly_b_low,vpmlx_e_up,vpmlx_e_low,vpmly_e_up,vpmly_e_low, &
        & vpml_exyz,coef_b_low,coef_b_up,xtrue,ytrue,dt,ci,&
        & inorder)
         ! transform the updated temp. box to Fourier space
         isign = -1
         call fft(vpml_exyz,isign,mixup,sct,tfft,indx,indy,inorder)
!         ! paste the temp. box the actual box field array
         exyz = 0.0
         call ilsremfield2(vpml_exyz,exyz,ffc,1,nx,ny,inorder)
         ! update B the second half time step
         if(yee==1) then
           call maxwelbyee(exyz,bxyz,cu,ffc,ci,dt,wf,wm,tfield,nx,ny,inorder)
         else
! shift the box array back to the integer grid points
     ...
     &,parmax,yee)
       ...
       if(yee==1) then
! shift the fields back to the integer grid points
         isign = -1
!         call fldshift(fxyze,exyz,bxyz,isign,nx,ny)
       endif
      endif
...
!     launch a laser in real space in lsrfxyze and lsrbxyze
     ...
     &lase0, laspol, lasstart,lasend,launchtype,lask, ntime, inorder, &
     &dt,ntlas,launchshape,lasw0,lasfocus,rayl,gamma,antlength,launchdir)
         !call pulsechop(lsrfxyze,lsrbxyze,10,0,nxe,nye,inorder)        
...
! vector potential diagnostic
     ...
     &modesxa,modesya,iua,narec,indx,indy,ntime,ndstyle,irc,inorder)
      if (irc==1) go to 2000
      if(parmax==1) then
! copy electric fields to force array
        isign = -1
        call emfield(fxyze,exyz,ffc,isign,nx,ny,inorder)
        if(yee==1.and.ntime==0) then
! shift initialized E and make satisfy finite diff. Gauss's law
! we're shifting E and B to their Yee potitions
          call fldmod_init(fxyze,exyz,bxyz,nx,ny,inorder)
!shift exyz and bxyz array back to the integer grid points for the pusher
!          isign = -1
!          call fldshift(fxyze,exyz,bxyz,isign,nx,ny)
        endif
       else
...
! copy magnetic field
      isign = -1
      call emfield(bxyze,bxyz,ffc,isign,nx,ny,inorder)
!take parallel part of force for debugging
!       call parforce(fxyze,fpar,nx,ny,inorder)
!       isign = 1
!       call fft(fpar,isign,mixup,sct,tfft,indx,indy,inorder)
!chop off high k for debugging
!       call modechop(fxyze,32,32,nx,ny,inorder)
!       call modechop(bxyze,32,32,nx,ny,inorder)
        !print*, fxyze(3,1,1)
        !print*, fxyze(3,1,nyh+1:ny)
...
! transform magnetic field to real space
      isign = 1
      call fft(bxyze,isign,mixup,sct,tfft,indx,indy,inorder)
      !call initpw2(fxyze,bxyze,nx,ny,nxe,nye,0.0,ci,isign)
...
!     cleanup
      ...
      print*, 'total time =', init_time + loop_time

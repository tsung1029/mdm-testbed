c 2d PIC library for pushing relativistic particles with darwin vector
c potential using hamiltonian formulation
...
c for relativistic particles.  the inverse gamma is also calculated.
c canonical momentum p(t-dt/2) = gamma*v(t-dt/2) + qi*A(t-dt/2)/(c*mi),
c where gamma = sqrt(1.+sum(pi**2)*ci*ci)
...
c part(9,n) = inverse gamma of particle n at t - dt/2
...
c nyv = second dimension of field array, must be >= ny+3
      ...
      real part, axy, qbm, ci, sx, sy, sz
      dimension part(idimp,nop)
      dimension axy(3,nxv,nyv)
      integer j, nn, mm, nl, np, ml, mp
      real ci2, p2, dxp, dyp, amx, amy, dxl, dyl, ax, ay, az, vx, vy, vz
...
c find inverse gamma
   ...
   10 continue
      ...
      subroutine GRHJPOST2(part,fxy,axy,daxy,cu,qm,qbm,dt,ci,idimp,nop,n
     1xv,nyv)
c for 2-1/2d code, this subroutine calculates current density, using
c second-order spline interpolation for relativistic particles.
c scalar version using guard cells
c 391 flops/particle, 2 divides, 1 sqrt, 132 loads, 28 stores
...
c cu(i,n+1,m-1)=.25*qci*((.5+dx)**2)*(.5-dy)**2
c cu(i,n-1,m-1)=.25*qci*((.5-dx)**2)*(.5-dy)**2
c and qci = qm*gvj*gami, where j = x,y,z, for i = 1, 3
c where gvj = .5*(pj(t+dt/2)+pj(t-dt/2)) - (q/m)*A(t)/c
c where gami = 1./sqrt(1.+sum(gvi**2)*ci*ci)
...
c omxx = .5*(q/m)*(dAx/dx)*gami/c, omyx = .5*(q/m)*(dAy/dx)*gami/c,
c omxy = .5*(q/m)*(dAx/dy)*gami/c, omzx = .5*(q/m)*(dAz/dx)*gami/c,
c omzy = .5*(q/m)*(dAz/dy)*gami/c
...
c part(7,n) = canonical momentum py of particle n at t - dt/2
c part(8,n) = canonical momentum pz of particle n at t - dt/2
c part(9,n) = inverse gamma of particle n at t
...
c nyv = second dimension of field arrays, must be >= ny+3
      implicit none
      integer idimp, nop, nxv, nyv
      real part, fxy, axy, daxy, cu, qm, qbm, dt, ci
      dimension part(idimp,nop)
      dimension fxy(3,nxv,nyv), axy(3,nxv,nyv), daxy(5,nxv,nyv)
      dimension cu(3,nxv,nyv)
      integer j, nn, mm, nl, np, ml, mp
      real qtmh, ci2, gami, qtmg, p2, dxp, dyp, amx, amy, dxl, dyl
...
c find vector potential gradients at t
     ...
     3p))
c renormalize vector potential gradients
      gami = part(9,j)
      qtmg = qtmh*gami
...
c update inverse gamma
   ...
   10 continue
      ...
      subroutine GSRHJPOST2(part,fxy,axy,daxy,cu,qm,qbm,dt,ci,idimp,nop,
     1nxv,nxyv)
c for 2-1/2d code, this subroutine calculates current density, using
c second-order spline interpolation for relativistic particles.
...
c nxyv = actual dimension of field array, must be >= nxv*(ny+3)
      ...
      real qtmh, ci2, gami, qtmg, p2, dxn, dyn, dxp, dyp, amx, amy
      real dxl, dyl, dx, dy, dz, ax, ay, az
      real oxx, oyx, ozx, oxy, ozy, omxxt, omyxt, omzxt, omxyt, omzyt
      real px, py, pz, acx, acy, acz, rot1, rot2, rot3, rot4, rot5, rot6
...
c update inverse gamma
      ...
      subroutine GRHPUSH23(part,fxy,axy,daxy,qbm,dt,ci,ek,idimp,nop,nx,n
     1y,nxv,nyv)
...
c scalar version using guard cells
c 324 flops/particle, 3 divides, 1 sqrt, 105 loads, 8 stores
...
c omzy = .5*(q/m)*(dAz/dy)*gami/c
c where gami = 1./sqrt(1.+sum(gvi**2)*ci*ci)
c and gvj = .5*(pj(t+dt/2)+pj(t-dt/2)) - (q/m)*A(t)/c
c Fx = Ex - 2*(Ax*omxx + Ay*omyx + Az*omzx)/c
c Fy = Ey - 2*(Ax*omxy - Ay*omxx + Az*omzy)/c
c Fz = Ez
c Ex = fxy(1,x(t),y(t)), Ey = fxy(2,x(t),y(t)), Ez = fxy(3,x(t),y(t)),
...
c where vj(t) = gami*(.5*(pj(t+dt/2)+pj(t-dt/2)) - (q/m)*A(t)/c)
c fxy(i,x(t),y(t)), axy(i,x(t),y(t)), and daxy(i,x(t),y(t))
c are approximated by interpolation from the nearest grid points:
c fxy(1,x,y) = 
...
c ek = gami*sum(px(t)**2 + py(t)**2 + pz(t)**2)/(1. + gami)
c idimp = size of phase space = 9
c nop = number of particles
...
c nyv = second dimension of field arrays, must be >= ny+3
      ...
      real part, fxy, axy, daxy, qbm, dt, ci, ek
      dimension part(idimp,nop)
      dimension fxy(3,nxv,nyv), axy(3,nxv,nyv), daxy(5,nxv,nyv)
      integer j, nn, mm, nl, np, ml, mp
      real qtmh, ci2, gami, qtmg, p2, dxp, dyp, amx, amy, dxl, dyl
...
c normalize kinetic energy
      ...
      subroutine GSRHPUSH23(part,fxy,axy,daxy,qbm,dt,ci,ek,idimp,nop,nx,
     1ny,nxv,nxyv)
c for 2-1/2d code, this subroutine updates particle co-ordinates,
c canonical momenta, and velocities using leap-frog scheme in time and
c second-order spline interpolation in space, periodic boundaries
...
c scalar version using guard cells, integer conversion precalculation,
c and 1d addressing
c 323 flops/particle, 3 divides, 1 sqrt, 105 loads, 8 stores
c input: all, output: part, ek
c momentum equations used are:
c px(t+dt/2) = rot(1)*(px(t-dt/2) + .5*(q/m)*Fx*dt) +
c    rot(2)*(py(t-dt/2) + .5*(q/m)*Fy*dt) +
...
c normalize kinetic energy
   20 ek = ek + sum1
      ...
      subroutine GRHPUSHXH23(part,axy,qbm,dt,ci,idimp,nop,nx,ny,nxv,nyv)
...
c for relativistic particles
c scalar version using guard cells
c 104 flops/particle, 1 divide, 1 sqrt, 32 loads, 5 stores
...
c where vj(t+dt/2) = gami*gvj, gami = 1./sqrt(1.+sum(gvi**2)*ci*ci)
c and gvj = pj(t+dt/2) - (q/m)*A(t+dt/2)/c
...
c nyv = second dimension of field arrays, must be >= ny+3
      ...
      integer j, nn, mm, nl, np, ml, mp
      real dth, ci2, gami, p2, dxp, dyp, amx, amy, dxl, dyl, dx, dy
      real ax, ay, az, px, py, pz, edgelx, edgely, edgerx, edgery
      dth = 0.5*dt
      ci2 = ci*ci
c set boundary values
      edgelx = 0.
...
c set new position
   ...
   10 continue
      ...
      subroutine GSRHPUSHXH23(part,axy,qbm,dt,ci,idimp,nop,nx,ny,nxv,nxy
     1v)
c for 2-1/2d code, this subroutine updates particle co-ordinates and
c velocities using leap-frog scheme in time and second-order spline
c interpolation in space, periodic boundaries
...
c nxyv = actual dimension of field array, must be >= nxv*(ny+3)
      ...
      integer nop1, j, nnn, mmn, nn, mm, ml, mn, mp
      real dth, ci2, gami, p2, dxn, dyn, dxp, dyp, amx, amy, dxl, dyl
      real dx, dy, ax, ay, az
...
c set new position
      ...
      subroutine GRHPUSHX23(part,axy,qbm,dt,ci,idimp,nop,nx,ny,nxv,nyv)
c for 2-1/2d code, this subroutine updates particle co-ordinates and
c velocities using leap-frog scheme in time and second-order spline
c interpolation in space, periodic boundaries
c for relativistic particles
c scalar version using guard cells
c 103 flops/particle, 1 divide, 1 sqrt, 32 loads, 6 stores
c input: all, output: part
c position equations used are:
...
c nyv = second dimension of field arrays, must be >= ny+3
      ...
      dimension part(idimp,nop), axy(3,nxv,nyv)
      integer j, nn, mm, nl, np, ml, mp
      real ci2, gami, p2, dxp, dyp, amx, amy, dxl, dyl, dx, dy
      real ax, ay, az, px, py, pz, edgelx, edgely, edgerx, edgery
      ci2 = ci*ci
c set boundary values
      edgelx = 0.
...
c new position at time t + dt
      dx = part(1,j) + (ax - 0.5*part(3,j))*dt
      dy = part(2,j) + (ay - 0.5*part(4,j))*dt
c store momentum and inverse gamma at time t + dt/2
      part(3,j) = px
      part(4,j) = py
      part(5,j) = pz
      part(9,j) = gami
...
c set new position
   ...
   10 continue
      ...
      subroutine GSRHPUSHX23(part,axy,qbm,dt,ci,idimp,nop,nx,ny,nxv,nxyv
     1)
c for 2-1/2d code, this subroutine updates particle co-ordinates and
c velocities using leap-frog scheme in time and second-order spline
c interpolation in space, periodic boundaries
...
c nxyv = actual dimension of field array, must be >= nxv*(ny+3)
      ...
      dimension part(idimp,nop), axy(3,nxyv)
      integer nop1, j, nnn, mmn, nn, mm, ml, mn, mp
      real ci2, gami, p2, dxn, dyn, dxp, dyp, amx, amy, dxl, dyl
      real dx, dy, ax, ay, az
      real px, py, pz, edgelx, edgely, edgerx, edgery
      if (nop.lt.1) return
c begin first particle
...
c velocity at time t + dt/2
      ...
      ay = gami*py
c new position at time t + dt
      dx = part(1,nop) + (ax - 0.5*part(3,j))*dt
      dy = part(2,nop) + (ay - 0.5*part(4,j))*dt
      part(3,nop) = px
      part(4,nop) = py
      part(5,nop) = pz
      part(9,nop) = gami
...
c part(7,n) = inverse gamma of particle n at t - dt/2
c axy(1,j+1,k+1) = x component of vector potential at grid (j,k)
c axy(2,j+1,k+1) = y component of vector potential at grid (j,k)
c qbm = particle charge/mass ratio
c ci = reciprical of velocity of light
c sx/sy = x/ycomponents of field momentum
c idimp = size of phase space = 7
c nop = number of particles
c nxv = first dimension of field array, must be >= nx+3
c nyv = second dimension of field array, must be >= ny+3
      implicit none
      integer idimp, nop, nxv, nyv
      real part, axy, qbm, ci, sx, sy
      dimension part(idimp,nop)
      dimension axy(2,nxv,nyv)
      integer j, nn, mm, nl, np, ml, mp
      real ci2, p2, dxp, dyp, amx, amy, dxl, dyl, ax, ay, vx, vy
...
c find inverse gamma
      p2 = vx*vx + vy*vy
      part(7,j)  = 1.0/sqrt(1.0 + p2*ci2)
   10 continue
      sx = sum1
      sy = sum2
      return
      end
      subroutine GRHJPOST22(part,fxy,axy,daxy,cu,qm,qbm,dt,ci,idimp,nop,
     1nxv,nyv)
c for 2d code, this subroutine calculates current density, using
c second-order spline interpolation for relativistic particles.
c scalar version using guard cells
c 265 flops/particle, 2 divides, 1 sqrt, 86 loads, 19 stores
c input: all, output: cu
c current density is approximated by values at the nearest grid points
c cu(i,n,m)=qci*(.75-dx**2)*(.75-dy**2)
c cu(i,n+1,m)=.5*qci*((.5+dx)**2)*(.75-dy**2)
...
c cu(i,n+1,m-1)=.25*qci*((.5+dx)**2)*(.5-dy)**2
c cu(i,n-1,m-1)=.25*qci*((.5-dx)**2)*(.5-dy)**2
c and qci = qm*gvj*gami, where j = x,y,z, for i = 1, 2
c where gvj = .5*(pj(t+dt/2)+pj(t-dt/2)) - (q/m)*A(t)/c
c where gami = 1./sqrt(1.+sum(gvi**2)*ci*ci)
c momentum equations at t=t+dt/2 are calculated from:
c px(t+dt/2) = rot(1)*(px(t-dt/2) + .5*(q/m)*Fx*dt) +
...
c part(5,n) = canonical momentum px of particle n at t - dt/2
c part(6,n) = canonical momentum py of particle n at t - dt/2
c part(7,n) = inverse gamma of particle n at t
c fxy(i,j+1,k+1) = ith component of force/charge at grid (j,k)
c axy(i,j+1,k+1) = ith component of vector potential at grid (j,k)
c daxy(i,j+1,k+1) = ith component of derivative of vector potential
c at grid (j,k)
...
c update inverse gamma
   ...
   10 continue
      ...
      subroutine GSRHJPOST22(part,fxy,axy,daxy,cu,qm,qbm,dt,ci,idimp,nop
     1,nxv,nxyv)
c for 2d code, this subroutine calculates current density, using
c second-order spline interpolation for relativistic particles.
c scalar version using guard cells, integer conversion precalculation,
...
c update inverse gamma
      ...
      subroutine GRHPUSH22(part,fxy,axy,daxy,qbm,dt,ci,ek,idimp,nop,nx,n
     1y,nxv,nyv)
...
c for relativistic particles
c scalar version using guard cells
c 223 flops/particle, 3 divides, 1 sqrt, 68 loads, 7 stores
c input: all, output: part, ek
c momentum equations used are:
c px(t+dt/2) = rot(1)*(px(t-dt/2) + .5*(q/m)*Fx*dt) +
c    rot(2)*(py(t-dt/2) + .5*(q/m)*Fy*dt) + .5*(q/m)*Fx*dt)
...
c ci = reciprical of velocity of light
c kinetic energy/mass at time t is also calculated, using
c ek = gami*sum(px(t)**2 + py(t)**2)/(1. + gami)
c idimp = size of phase space = 7
c nop = number of particles
c nx/ny = system length in x/y direction
c nxv = first dimension of field arrays, must be >= nx+3
c nyv = second dimension of field arrays, must be >= ny+3
      ...
      dimension fxy(2,nxv,nyv), axy(2,nxv,nyv), daxy(3,nxv,nyv)
      integer j, nn, mm, nl, np, ml, mp
      real dth, qtmh, ci2, gami, qtmg, p2, dxp, dyp, amx, amy
      real dxl, dyl, dx, dy, ax, ay
      real oxx, oyx, oxy, omxxt, omyxt, omxyt
      real px, py, acx, acy, rot0, rot1, rot2, rot3, rot4, anorm
...
c normalize kinetic energy
      ...
      subroutine GSRHPUSH22(part,fxy,axy,daxy,qbm,dt,ci,ek,idimp,nop,nx,
     1ny,nxv,nxyv)
c for 2d code, this subroutine updates particle co-ordinates,
c canonical momenta, and velocities using leap-frog scheme in time and
c second-order spline interpolation in space, periodic boundaries
...
c nxyv = actual dimension of field array, must be >= nxv*(ny+3)
      ...
      dimension fxy(2,nxyv), axy(2,nxyv), daxy(3,nxyv)
      integer nop1, j, nnn, mmn, nn, mm, ml, mn, mp
      real dth, qtmh, ci2, gami, qtmg, p2, dxn, dyn, dxp, dyp
      real amx, amy, dxl, dyl, dx, dy, ax, ay
      real oxx, oyx, oxy, omxxt, omyxt, omxyt
      real px, py, acx, acy, rot0, rot1, rot2, rot3, rot4, anorm
      real edgelx, edgely, edgerx, edgery
      double precision sum1
...
c normalize kinetic energy
   20 ek = ek + .5*sum1
      ...
      subroutine GRHPUSHXH22(part,axy,qbm,dt,ci,idimp,nop,nx,ny,nxv,nyv)
c for 2d code, this subroutine updates particle co-ordinates and
c velocities using leap-frog scheme in time and second-order spline
c interpolation in space, periodic boundaries
c for relativistic particles
c scalar version using guard cells
c 81 flops/particle, 1 divide, 1 sqrt, 22 loads, 4 stores
c input: all, output: part
c position equations used are:
c x(t+dt/2) = x(t+dt/2) + .5*(vxnew(t+dt/2) - vxold(t+dt/2))*dt
c y(t+dt/2) = y(t+dt/2) + .5*(vynew(t+dt/2) - vxold(t+dt/2))*dt
c where vj(t+dt/2) = gami*gvj, gami = 1./sqrt(1.+sum(gvi**2)*ci*ci)
c and gvj = pj(t+dt/2) - (q/m)*A(t+dt/2)/c
c axy(i,x(t),y(t)) is approximated by interpolation from the nearest
c grid points:
...
c nyv = second dimension of field arrays, must be >= ny+3
      ...
      integer j, nn, mm, nl, np, ml, mp
      real dth, ci2, gami, p2, dxp, dyp, amx, amy, dxl, dyl, dx, dy
      real ax, ay, px, py, edgelx, edgely, edgerx, edgery
      dth = 0.5*dt
      ci2 = ci*ci
c set boundary values
      edgelx = 0.
...
c set new position
   ...
   10 continue
      ...
      subroutine GSRHPUSHXH22(part,axy,qbm,dt,ci,idimp,nop,nx,ny,nxv,nxy
     1v)
c for 2d code, this subroutine updates particle co-ordinates and
c velocities using leap-frog scheme in time and second-order spline
c interpolation in space, periodic boundaries
...
c set new position
      ...
      subroutine GRHPUSHX22(part,axy,qbm,dt,ci,idimp,nop,nx,ny,nxv,nyv)
c for 2d code, this subroutine updates particle co-ordinates and
c velocities using leap-frog scheme in time and second-order spline
c interpolation in space, periodic boundaries
c for relativistic particles
c scalar version using guard cells
c 81 flops/particle, 1 divide, 1 sqrt, 22 loads, 5 stores
c input: all, output: part
c position equations used are:
c x(t+dt/2) = x(t+dt/2) + (vxnew(t+dt/2) - .5*vxold(t+dt/2))*dt
c y(t+dt/2) = y(t+dt/2) + (vynew(t+dt/2) - .5*vyold(t+dt/2))*dt
...
c nyv = second dimension of field arrays, must be >= ny+3
      ...
      dimension part(idimp,nop), axy(2,nxv,nyv)
      integer j, nn, mm, nl, np, ml, mp
      real ci2, gami, p2, dxp, dyp, amx, amy, dxl, dyl, dx, dy, ax, ay
      real px, py, edgelx, edgely, edgerx, edgery
      ci2 = ci*ci
c set boundary values
      edgelx = 0.
...
c set new position
   ...
   10 continue
      ...
      subroutine GSRHPUSHX22(part,axy,qbm,dt,ci,idimp,nop,nx,ny,nxv,nxyv
     1)
c for 2d code, this subroutine updates particle co-ordinates and
c velocities using leap-frog scheme in time and second-order spline
c interpolation in space, periodic boundaries
...
c velocity at time t + dt/2
      ...
      ay = gami*py
c new position at time t + dt
      dx = part(1,nop) + (ax - 0.5*part(3,nop))*dt
      dy = part(2,nop) + (ay - 0.5*part(4,nop))*dt
      part(3,nop) = px
      part(4,nop) = py
      part(7,nop) = gami
c periodic boundary conditions
...
c set new position
      ...
      subroutine RCR8PCM23L(part,axy,qbm,ci,sx,sy,sz,idimp,nop,nxv,nyv)
c for 2-1/2d code, this subroutine calculates the canonical momentum of
c a particle from the vector potential at t - dt/2, and adds this
...
c nyv = second dimension of field array, must be >= ny+1
      ...
      real ci2, p2, dxp, dyp, amx, amy, ax, ay, az, vx, vy, vz
      double precision sum1, sum2, sum3
      ci2 = ci*ci
      sum1 = 0.0d0
      sum2 = 0.0d0
...
c find inverse gamma
   ...
   10 continue
      ...
      subroutine GRHJPOST2L(part,fxy,axy,daxy,cu,qm,qbm,dt,ci,idimp,nop,
     1nxv,nyv)
c for 2-1/2d code, this subroutine calculates current density, using
c first-order linear interpolation for relativistic particles.
c scalar version using guard cells
c 219 flops/particle, 2 divides, 1 sqrt, 62 loads, 13 stores
c input: all, output: cu
c current density is approximated by values at the nearest grid points
...
c nyv = second dimension of field arrays, must be >= ny+1
      ...
      dimension cu(3,nxv,nyv)
      integer j, nn, mm, np, mp
      real qtmh, ci2, gami, qtmg, p2, dxp, dyp, amx, amy
      real dx, dy, dz, ax, ay, az
      real oxx, oyx, ozx, oxy, ozy, omxxt, omyxt, omzxt, omxyt, omzyt
      real px, py, pz, acx, acy, acz, rot1, rot2, rot3, rot4, rot5, rot6
      real anorm
...
c update inverse gamma
   ...
   10 continue
      ...
      subroutine GSRHJPOST2L(part,fxy,axy,daxy,cu,qm,qbm,dt,ci,idimp,nop
     1,nxv,nxyv)
c for 2-1/2d code, this subroutine calculates current density, using
c first-order linear interpolation for relativistic particles.
c scalar version using guard cells, integer conversion precalculation,
...
c update inverse gamma
      ...
      subroutine GRHPUSH23L(part,fxy,axy,daxy,qbm,dt,ci,ek,idimp,nop,nx,
     1ny,nxv,nyv)
c for 2-1/2d code, this subroutine updates particle co-ordinates,
c canonical momenta, and velocities using leap-frog scheme in time and
...
c for relativistic particles
c scalar version using guard cells
c 197 flops/particle, 3 divides, 1 sqrt, 50 loads, 8 stores
c input: all, output: part, ek
c momentum equations used are:
c px(t+dt/2) = rot(1)*(px(t-dt/2) + .5*(q/m)*Fx*dt) +
c    rot(2)*(py(t-dt/2) + .5*(q/m)*Fy*dt) +
...
c normalize kinetic energy
      ...
      subroutine GSRHPUSH23L(part,fxy,axy,daxy,qbm,dt,ci,ek,idimp,nop,nx
     1,ny,nxv,nxyv)
c for 2-1/2d code, this subroutine updates particle co-ordinates,
c canonical momenta, and velocities using leap-frog scheme in time and
c first-order linear interpolation in space, periodic boundaries
...
c normalize kinetic energy
   20 ek = ek + .5*sum1
      ...
      subroutine GRHPUSHXH23L(part,axy,qbm,dt,ci,idimp,nop,nx,ny,nxv,nyv
     1)
c for 2-1/2d code, this subroutine updates particle co-ordinates and
...
c for relativistic particles
c scalar version using guard cells
c 57 flops/particle, 1 divide, 1 sqrt, 19 loads, 5 stores
c input: all, output: part
c position equations used are:
c x(t+dt/2) = x(t+dt/2) + .5*(vxnew(t+dt/2) - vxold(t+dt/2))*dt
c y(t+dt/2) = y(t+dt/2) + .5*(vynew(t+dt/2) - vxold(t+dt/2))*dt
...
c nyv = second dimension of field arrays, must be >= ny+1
      ...
      dimension part(idimp,nop), axy(3,nxv,nyv)
      integer j, nn, mm, np, mp
      real dth, ci2, gami, p2, dxp, dyp, amx, amy, dx, dy, ax, ay, az
      real px, py, pz, edgelx, edgely, edgerx, edgery
      dth = 0.5*dt
      ci2 = ci*ci
c set boundary values
...
c set new position
   ...
   10 continue
      ...
      subroutine GSRHPUSHXH23L(part,axy,qbm,dt,ci,idimp,nop,nx,ny,nxv,nx
     1yv)
c for 2-1/2d code, this subroutine updates particle co-ordinates and
c velocities using leap-frog scheme in time and first-order linear
c interpolation in space, periodic boundaries
...
c nxyv = actual dimension of field array, must be >= nxv*(ny+1)
      ...
      dimension part(idimp,nop), axy(3,nxyv)
      integer nop1, j, nnn, mmn, nn, mm, mp
      real dth, ci2, gami, p2, dxn, dyn, dxp, dyp, amx, amy, dx, dy
      real ax, ay, az, px, py, pz, edgelx, edgely, edgerx, edgery
      if (nop.lt.1) return
c begin first particle
      nnn = part(1,1)
...
c set new position
      ...
      subroutine GRHPUSHX23L(part,axy,qbm,dt,ci,idimp,nop,nx,ny,nxv,nyv)
c for 2-1/2d code, this subroutine updates particle co-ordinates and
c velocities using leap-frog scheme in time and first-order linear
c interpolation in space, periodic boundaries
c for relativistic particles
c scalar version using guard cells
c 56 flops/particle, 1 divide, 1 sqrt, 19 loads, 6 stores
c input: all, output: part
c position equations used are:
c x(t+dt/2) = x(t+dt/2) + (vxnew(t+dt/2) - .5*vxold(t+dt/2))*dt
c y(t+dt/2) = y(t+dt/2) + (vynew(t+dt/2) - .5*vyold(t+dt/2))*dt
...
c nyv = second dimension of field arrays, must be >= ny+1
      ...
      dimension part(idimp,nop), axy(3,nxv,nyv)
      integer j, nn, mm, np, mp
      real ci2, gami, p2, dxp, dyp, amx, amy, dx, dy, ax, ay, az
      real px, py, pz, edgelx, edgely, edgerx, edgery
      ci2 = ci*ci
c set boundary values
      edgelx = 0.
...
c set new position
   ...
   10 continue
      ...
      subroutine GSRHPUSHX23L(part,axy,qbm,dt,ci,idimp,nop,nx,ny,nxv,nxy
     1v)
c for 2-1/2d code, this subroutine updates particle co-ordinates and
c velocities using leap-frog scheme in time and first-order linear
c interpolation in space, periodic boundaries
...
c nxyv = actual dimension of field array, must be >= nxv*(ny+1)
      ...
      dimension part(idimp,nop), axy(3,nxyv)
      integer nop1, j, nnn, mmn, nn, mm, mp
      real ci2, gami, p2, dxn, dyn, dxp, dyp, amx, amy, dx, dy
      real ax, ay, az, px, py, pz, edgelx, edgely, edgerx, edgery
      if (nop.lt.1) return
c begin first particle
      nnn = part(1,1)
...
c nyv = second dimension of field array, must be >= ny+1
      ...
      dimension axy(2,nxv,nyv)
      integer j, nn, mm, np, mp
      real ci2, p2, dxp, dyp, amx, amy, ax, ay, vx, vy
      double precision sum1, sum2
      ci2 = ci*ci
      sum1 = 0.0d0
      sum2 = 0.0d0
...
c find inverse gamma
   ...
   10 continue
      ...
      subroutine GRHJPOST22L(part,fxy,axy,daxy,cu,qm,qbm,dt,ci,idimp,nop
     1,nxv,nyv)
c for 2d code, this subroutine calculates current density, using
c first-order linear interpolation for relativistic particles.
c scalar version using guard cells
c 144 flops/particle, 2 divides, 1 sqrt, 39 loads, 9 stores
c input: all, output: cu
c current density is approximated by values at the nearest grid points
c cu(i,n,m)=qci*(1.-dx)*(1.-dy)
c cu(i,n+1,m)=qci*dx*(1.-dy)
...
c nyv = second dimension of field arrays, must be >= ny+1
      ...
      dimension cu(2,nxv,nyv)
      integer j, nn, mm, np, mp
      real qtmh, ci2, gami, qtmg, p2, dxp, dyp, amx, amy, dx, dy, ax, ay
      real oxx, oyx, oxy, omxxt, omyxt, omxyt
      real px, py, acx, acy, rot0, rot1, rot2, rot3, rot4, anorm
      qtmh = .5*qbm*dt
      ci2 = ci*ci
...
c update inverse gamma
   ...
   10 continue
      ...
      subroutine GSRHJPOST22L(part,fxy,axy,daxy,cu,qm,qbm,dt,ci,idimp,no
     1p,nxv,nxyv)
c for 2d code, this subroutine calculates current density, using
c first-order linear interpolation for relativistic particles.
c scalar version using guard cells, integer conversion precalculation,
...
c update inverse gamma
      ...
      subroutine GRHPUSH22L(part,fxy,axy,daxy,qbm,dt,ci,ek,idimp,nop,nx,
     1ny,nxv,nyv)
c for 2d code, this subroutine updates particle co-ordinates,
c canonical momenta, and velocities using leap-frog scheme in time and
c first-order linear interpolation in space, periodic boundaries
c for relativistic particles
c scalar version using guard cells
c 130 flops/particle, 3 divides, 1 sqrt, 33 loads, 6 stores
c input: all, output: part, ek
c momentum equations used are:
c px(t+dt/2) = rot(1)*(px(t-dt/2) + .5*(q/m)*Fx*dt) +
c    rot(2)*(py(t-dt/2) + .5*(q/m)*Fy*dt) + .5*(q/m)*Fx*dt)
...
c normalize kinetic energy
      ...
      subroutine GSRHPUSH22L(part,fxy,axy,daxy,qbm,dt,ci,ek,idimp,nop,nx
     1,ny,nxv,nxyv)
c for 2d code, this subroutine updates particle co-ordinates,
c canonical momenta, and velocities using leap-frog scheme in time and
c first-order linear interpolation in space, periodic boundaries
...
c    rot(3) = 2*omxy/(1 - omxx**2 - omyx*omxy)
c    rot(4) = ((1 - omxx)**2 + omyx*omxy)/(1 - omxx**2 - omyx*omxy)
c omxx = .5*(q/m)*(dAx/dx)*gami/c, omyx = .5*(q/m)*(dAy/dx)v/c,
c omxy = .5*(q/m)*(dAx/dy)*gami/c,
c where gami = 1./sqrt(1.+sum(gvi**2)*ci*ci)
c and gvj = .5*(pj(t+dt/2)+pj(t-dt/2)) - (q/m)*A(t)/c
c Fx = Ex - 2*(Ax*omxx + Ay*omyx)/c
...
c normalize kinetic energy
   20 ek = ek + .5*sum1
      ...
      subroutine GRHPUSHXH22L(part,axy,qbm,dt,ci,idimp,nop,nx,ny,nxv,nyv
     1)
c for 2d code, this subroutine updates particle co-ordinates and
c velocities using leap-frog scheme in time and first-order linear
c interpolation in space, periodic boundaries
c for relativistic particles
c scalar version using guard cells
c 43 flops/particle, 1 divide, 1 sqrt, 14 loads, 4 stores
c input: all, output: part
c position equations used are:
c x(t+dt/2) = x(t+dt/2) + .5*(vxnew(t+dt/2) - vxold(t+dt/2))*dt
c y(t+dt/2) = y(t+dt/2) + .5*(vynew(t+dt/2) - vxold(t+dt/2))*dt
...
c nyv = second dimension of field arrays, must be >= ny+1
      ...
      dimension part(idimp,nop), axy(2,nxv,nyv)
      integer j, nn, mm, np, mp
      real dth, ci2, gami, p2, dxp, dyp, amx, amy, dx, dy, ax, ay
      real px, py, edgelx, edgely, edgerx, edgery
      dth = 0.5*dt
      ci2 = ci*ci
c set boundary values
...
c set new position
   ...
   10 continue
      ...
      subroutine GSRHPUSHXH22L(part,axy,qbm,dt,ci,idimp,nop,nx,ny,nxv,nx
     1yv)
c for 2d code, this subroutine updates particle co-ordinates and
c velocities using leap-frog scheme in time and first-order linear
c interpolation in space, periodic boundaries
...
c set new position
      ...
      subroutine GRHPUSHX22L(part,axy,qbm,dt,ci,idimp,nop,nx,ny,nxv,nyv)
c for 2d code, this subroutine updates particle co-ordinates and
c velocities using leap-frog scheme in time and first-order linear
c interpolation in space, periodic boundaries
c for relativistic particles
c scalar version using guard cells
c 43 flops/particle, 1 divide, 1 sqrt, 14 loads, 5 stores
c input: all, output: part
c position equations used are:
c x(t+dt/2) = x(t+dt/2) + (vxnew(t+dt/2) - .5*vxold(t+dt/2))*dt
c y(t+dt/2) = y(t+dt/2) + (vynew(t+dt/2) - .5*vyold(t+dt/2))*dt
...
c nyv = second dimension of field arrays, must be >= ny+1
      ...
      dimension part(idimp,nop), axy(2,nxv,nyv)
      integer j, nn, mm, np, mp
      real ci2, gami, p2, dxp, dyp, amx, amy, dx, dy, ax, ay
      real px, py, edgelx, edgely, edgerx, edgery
      ci2 = ci*ci
c set boundary values
      edgelx = 0.
...
c set new position
   ...
   10 continue
      ...
      subroutine GSRHPUSHX22L(part,axy,qbm,dt,ci,idimp,nop,nx,ny,nxv,nxy
     1v)
c for 2d code, this subroutine updates particle co-ordinates and
c velocities using leap-frog scheme in time and first-order linear
c interpolation in space, periodic boundaries
...
c nxyv = actual dimension of field array, must be >= nxv*(ny+1)
      ...
      dimension part(idimp,nop), axy(2,nxyv)
      integer nop1, j, nnn, mmn, nn, mm, mp
      real ci2, gami, p2, dxn, dyn, dxp, dyp, amx, amy, dx, dy, ax, ay
      real px, py, edgelx, edgely, edgerx, edgery
      if (nop.lt.1) return
c begin first particle
      nnn = part(1,1)
...
c set new position
      ...
      subroutine RCR8PCM23GL(part,axy,sctx,qbm,ci,sx,sy,sz,idimp,nop,nx,
     1ny,nxvh,nyv)
c for 2-1/2d code, this subroutine calculates the canonical momentum of
c a particle from the vector potential at t - dt/2, and adds this
c information to the particle array, for relativistic particles
c using gridless spectral version with periodic boundaries,
c the inverse gamma is also calculated.
c canonical momentum p(t-dt/2) = gamma*v(t-dt/2) + qi*A(t-dt/2)/(c*mi),
c where gamma = sqrt(1.+sum(pi**2)*ci*ci)
...
c nyv = second dimension of field arrays, must be >= ny
      ...
      dimension sctx(nxvh)
      integer i, j, k, k1, nxh, nyh, ny2
      real dnx, dny, dkx, dky, at1, at3, qtmh, ci2, gami, qtmg, p2, qmn
...
c mode numbers ky = 0, ny/2
  ...
  120 continue
      ...
      subroutine GRHPUSH23GL(part,fxy,axy,daxy,sctx,qbm,dt,ci,ek,idimp,n
     1op,nx,ny,nxvh,nyv)
c for 2-1/2d code, this subroutine updates particle co-ordinates,
...
c for relativistic particles
c baseline scalar version
c 100*(NX/2)*(NY/2) + 1 divide + 82 flops/particle,
c 22*NX*NY/2 + 6 loads and 8 stores
c input: all, output: part, ek
c momentum equations used are:
c px(t+dt/2) = rot(1)*(px(t-dt/2) + .5*(q/m)*Fx*dt) +
c    rot(2)*(py(t-dt/2) + .5*(q/m)*Fy*dt) +
...
c nyv = second dimension of field arrays, must be >= ny
      ...
      dimension sctx(nxvh)
      integer i, j, k, k1, nxh, nyh, ny2
      real dnx, dny, dkx, dky, at1, at3, dth, qtmh, ci2, gami, qtmg, p2
...
c normalize kinetic energy
      ...
      subroutine GRHPUSHXH23GL(part,axy,sctx,qbm,dt,ci,idimp,nop,nx,ny,n
     1xvh,nyv)
c for 2-1/2d code, this subroutine updates particle co-ordinates and
...
c for relativistic particles
c baseline scalar version
c 27*(NX/2)*(NY/2) + 1 divide, 1 sqrt, 12 flops/particle,
...
c nyv = second dimension of field arrays, must be >= ny
      ...
      dimension sctx(nxvh)
      integer i, j, k, k1, nxh, nyh, ny2
      real dnx, dny, dkx, dky, at1, at3, dth, ci2, gami, p2
...
c set new position
   ...
   50 continue
      ...
      subroutine GRHPUSHX23GL(part,axy,sctx,qbm,dt,ci,idimp,nop,nx,ny,nx
     1vh,nyv)
c for 2-1/2d code, this subroutine updates particle co-ordinates and
c velocities using leap-frog scheme in time with
c gridless spectral version with periodic boundaries,
...
c 3*NX*NY/2 + 1 divide, 1 sqrt, 8 loads and 6 stores
c input: all, output: part
c position equations used are:
c x(t+dt/2) = x(t+dt/2) + (vxnew(t+dt/2) - .5*vxold(t+dt/2))*dt
c y(t+dt/2) = y(t+dt/2) + (vynew(t+dt/2) - .5*vyold(t+dt/2))*dt
...
c nyv = second dimension of field arrays, must be >= ny
      ...
      dimension sctx(nxvh)
      integer i, j, k, k1, nxh, nyh, ny2
      real dnx, dny, dkx, dky, at1, at3, ci2, gami, p2
      real dx, dy, ax, ay, az, edgelx, edgely, edgerx, edgery
      real px, py, pz
      double precision axl, ayl, azl, ox, oy, oz
      complex zt1, zt2, zt3, zt4
...
c velocity at time t + dt/2
      ...
      ay = gami*py
c new position at time t + dt
      dx = part(1,i) + (ax - 0.5*part(3,i))*dt
      dy = part(2,i) + (ay - 0.5*part(4,i))*dt
      part(3,i) = px
      part(4,i) = py
      part(5,i) = pz
      part(9,i) = gami

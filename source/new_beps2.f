...
! default unit numbers
      ...
      real :: qbme, qbmi, affp, qi0, omt, etx, we, wke
...
! initialize external magnetic field
      if (omt > 0) then
!        allocate(bxyze(3,nxe,nye))
         allocate(bxyze(1,nxe,nye))
         bxyze = 0.0
...
! transform force/charge to real space
      isign = 1
      call fft(fxye,isign,mixup,sct,tfft,indx,indy,inorder)
!     call fftn(fxye,isign,mixup,sct,tfft,indx,indy,inorder)
      call cguard(fxye,nx,ny,inorder)
! external pump
...
! push electrons
      if (omt <= 0) then
         call pushg(part,fxye,np,qbme,dt,ci,wke,tpush,nx,ny,ipbc,       &
     &relativity,inorder,popt)
      else
         call bpushg(part,fxye,bxyze,np,qbme,dt,ci,wke,tpush,nx,ny,ipbc,&
     &relativity,inorder,popt)
      endif
! debug: zero force
...
! push ions
     ...
     &relativity,inorder,popt)
            ...
            call bpushg(parti,fxye,bxyze,npi,qbmi,dt,ci,wki,tpushi,nx,ny&
...
! calculate ion and total momentum
      call imomtdiag(parti,qi,fxye,dt,rmass,pxi,pyi,pzi,wx,wy,wz,ntm,   &
...
! accumulate timings
      write (iuot,*) 'electrostatic code beps2'
